...
class DataSelect extends Component{
    ...
    renderSelectNodes=(list,seachText,selectKey)=>{
        return list.filter(row=>row.label.toLocaleLowerCase().indexOf(seachText.toLocaleLowerCase())>-1).map(row=>{
            const cls = classNames(styles.rowItem, {
                [styles.active]: row.value==selectKey,
            });    
            return <div onClick={()=>this.onSelect(row.value)} key={row.value} className={cls}>{row.label}</div>
        })
    }
                ...
                <Spin spinning={loading}>
                    <div className={styles.header}>
                        ...
                        <div className={styles.seach}> 
                            <Input onChange={this.onSeach} type={null} size="small" placeholder="输入关键词..." />
                            <span>
                                <a onClick={()=>{this.onCheckAll(true)}}>全选</a>
                                <a onClick={()=>{this.onCheckAll(false)}}>全否</a>
                            </span>
                        </div>
                        <div className={styles.body}>
                            {
                                list.length?<CheckboxGroup value={checkedList} onChange={this.onCheck} style={{padding:'0 5px',width:'100%' }}>
                                    {
                                        <Row>
                                            {list.filter(row=>row.label.toLocaleLowerCase().indexOf(seachText.toLocaleLowerCase())>-1).map((row, index) => {
                                                return (
